 ...
 * @author Samuel Audet
 */
public class FFmpegFrameGrabber0 extends FrameGrabber {
    ...
    public static FFmpegFrameGrabber0 createDefault(File deviceFile) throws Exception {
        return new FFmpegFrameGrabber0(deviceFile);
    }

    public static FFmpegFrameGrabber0 createDefault(String devicePath) throws Exception {
        return new FFmpegFrameGrabber0(devicePath);
    }

    public static FFmpegFrameGrabber0 createDefault(int deviceNumber) throws Exception {
        throw new Exception(FFmpegFrameGrabber0.class + " does not support device numbers.");
    }
        ...
        if (loadingException != null) {
            ...
            try {
                ...
                if (t instanceof Exception) {
                    ...
                    throw loadingException = new Exception("Failed to load " + FFmpegFrameGrabber0.class, t);
                }
            }
        }
    }
    ...
    public FFmpegFrameGrabber0(File file) {
        this(file.getAbsolutePath());
    }

    public FFmpegFrameGrabber0(String filename) {
    ...
    /**
     * Calls {@code FFmpegFrameGrabber(inputStream, Integer.MAX_VALUE - 8)} so that the whole input stream is seekable.
     */
    public FFmpegFrameGrabber0(InputStream inputStream) {
        this(inputStream, Integer.MAX_VALUE - 8);
    }

    /** Set maximumSize to 0 to disable seek and minimize startup time. */
    public FFmpegFrameGrabber0(InputStream inputStream, int maximumSize) {
     ...
     *
     * Care must be taken as this method may require unnecessary call of grabFrame(true, false, false, false, false)
     * with frameGrabbed set to true.
     *
     * @return (double) getSampleRate()) / samples_frame.nb_samples() if samples_frame.nb_samples() is not zero,
     *         otherwise return 0
     */
    ...
    /**
     * default override of super.setFrameNumber implies setting of a frame close to a video frame having that number
     */
    @Override
    ...
    /**
     * if there is video stream tries to seek to video frame with corresponding timestamp otherwise sets
     * super.frameNumber only because frameRate==0 if there is no video stream
     */
    ...
    /**
     * if there is audio stream tries to seek to audio frame with corresponding timestamp ignoring otherwise
     */
    ...
    /**
     * setTimestamp with possibility to select between old quick seek code or new code doing check of frame content. The
     * frame check can be useful with corrupted files, when seeking may end up with an empty frame not containing video
     * nor audio
     */
    ...
    /**
     * setTimestamp with resulting video frame type if there is a video stream. This should provide precise seek to a
     * video frame containing the requested timestamp in most cases.
     */
    ...
    /**
     * setTimestamp with resulting audio frame type if there is an audio stream. This should provide precise seek to an
     * audio frame containing the requested timestamp in most cases.
     */
    ...
    /**
     * setTimestamp with a priority the resulting frame should be: video (frameTypesToSeek contains only
     * Frame.Type.VIDEO), audio (frameTypesToSeek contains only Frame.Type.AUDIO), or any (frameTypesToSeek contains
     * both)
     */
    private synchronized void setTimestamp(long timestamp, EnumSet<Frame.Type> frameTypesToSeek) throws Exception {
        ...
        if (oc == null) {
            ...
            if ((ret =
                avformat_seek_file(oc, -1, Long.MIN_VALUE, early_ts, Long.MAX_VALUE, AVSEEK_FLAG_BACKWARD)) < 0) {
                throw new Exception(
            ...
            if (frameTypesToSeek != null // new code providing check of frame content while seeking to the timestamp
                ...
                if (has_video || has_audio) {
                    ...
                    seekFrame = grabFrame(frameTypesToSeek.contains(Frame.Type.AUDIO),
                        frameTypesToSeek.contains(Frame.Type.VIDEO), false, false, false);
                    if (seekFrame == null)
                        return;
    ...
    /**
     * Returns {@code (int) Math.round(getLengthInTime() * getFrameRate() / 1000000L)}, which is an approximation in
     * general.
     */
    ...
    public synchronized Frame grabFrame(boolean doAudio, boolean doVideo, boolean doProcessing, boolean keyFrames,
        boolean doData) throws Exception {
        try (PointerScope scope = new PointerScope()) {
