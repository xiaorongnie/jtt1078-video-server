...
public abstract class G726 {
    ...
    private static final /*short*/int[] power2 =
        {1, 2, 4, 8, 0x10, 0x20, 0x40, 0x80, 0x100, 0x200, 0x400, 0x800, 0x1000, 0x2000, 0x4000};

    /**
     * Quantizes the input val against the table of size short integers. It returns i if table[i-1]<=val<table[i].
     * <p>
    ...
    /**
     * Given a raw sample, 'd', of the difference signal and a quantization step size scale factor, 'y', this routine
     * returns the ADPCM codeword to which that sample gets quantized. The step size scale factor division operation is
     * done in the log base 2 domain as a subtraction. <br>
     * 
     * @param d
    ...
    /**
     * Returns reconstructed difference signal 'dq' obtained from codeword 'i' and quantization step size scale factor
     * 'y'. Multiplication is performed in log base 2 domain as addition.
     * 
     * @param sign
    ...
    /**
     * At the end of ADPCM decoding, it simulates an encoder which may be receiving the output of this decoder as a
     * tandem process. If the output of the simulated encoder differs from the input to this decoder, the decoder output
    ...
    public static int signedIntLittleEndian(byte hi_b, byte lo_b) {
        int sign_bit = hi_b >> 7;
        return (sign_bit == 0) ? (unsignedInt(hi_b) << 8) + unsignedInt(lo_b)
            : (-1 ^ 0x7FFF) ^ (((unsignedInt(hi_b) & 0x7F) << 8) + unsignedInt(lo_b));
    }
    ...
    /**
     * Encodes the input vale of linear PCM, A-law or u-law data sl and returns the resulting code. -1 is returned for
     * unknown input coding value.
     */
    public abstract int encode(int sl, int in_coding);

    /**
     * Encodes the input chunk in_buff of linear PCM, A-law or u-law data and returns the G726 encoded chuck into
     * out_buff. <br>
     * It returns the actual size of the output data, or -1 in case of unknown in_coding value.
     */
    public abstract int encode(byte[] in_buff, int in_offset, int in_len, int in_coding, byte[] out_buff,
        int out_offset);

    /**
     * Decodes a 4-bit code of G.72x encoded data of i and returns the resulting linear PCM, A-law or u-law value.
     * return -1 for unknown out_coding value.
     */
    public abstract int decode(int i, int out_coding);

    /**
     * Decodes the input chunk in_buff of G726 encoded data and returns the linear PCM, A-law or u-law chunk into
     * out_buff. <br>
     * It returns the actual size of the output data, or -1 in case of unknown out_coding value.
     */
    public abstract int decode(byte[] in_buff, int in_offset, int in_len, int out_coding, byte[] out_buff,
        int out_offset);
