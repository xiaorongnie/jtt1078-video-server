package com.transcodegroup.jtt1078.codec.ffmpeg.javacv;
 ...
 * @author Samuel Audet
 */
public class AudioFileRecorder extends FrameRecorder {
    ...
    public static AudioFileRecorder createDefault(File f, int w, int h) throws Exception {
        return new AudioFileRecorder(f, w, h);
    }

    public static AudioFileRecorder createDefault(String f, int w, int h) throws Exception {
        return new AudioFileRecorder(f, w, h);
    }
        ...
        if (loadingException != null) {
            ...
            try {
                ...
                if (t instanceof Exception) {
                    ...
                    throw loadingException = new Exception("Failed to load " + AudioFileRecorder.class, t);
                }
            }
        }
    }
    ...
    public AudioFileRecorder(File file, int audioChannels) {
        this(file, 0, 0, audioChannels);
    }

    public AudioFileRecorder(String filename, int audioChannels) {
        this(filename, 0, 0, audioChannels);
    }

    public AudioFileRecorder(File file, int imageWidth, int imageHeight) {
        this(file, imageWidth, imageHeight, 0);
    }

    public AudioFileRecorder(String filename, int imageWidth, int imageHeight) {
        this(filename, imageWidth, imageHeight, 0);
    }

    public AudioFileRecorder(File file, int imageWidth, int imageHeight, int audioChannels) {
        this(file.getAbsolutePath(), imageWidth, imageHeight, audioChannels);
    }

    public AudioFileRecorder(String filename, int imageWidth, int imageHeight, int audioChannels) {
    ...
    public AudioFileRecorder(OutputStream outputStream, int audioChannels) {
        this(outputStream.toString(), audioChannels);
        this.outputStream = outputStream;
        this.closeOutputStream = true;
    }

    public AudioFileRecorder(OutputStream outputStream, int imageWidth, int imageHeight) {
        this(outputStream.toString(), imageWidth, imageHeight);
        this.outputStream = outputStream;
        this.closeOutputStream = true;
    }

    public AudioFileRecorder(OutputStream outputStream, int imageWidth, int imageHeight, int audioChannels) {
    ...
    public void record(Frame frame) throws Exception {
        record(frame,
            frame != null && frame.opaque instanceof AVFrame ? ((AVFrame)frame.opaque).format() : AV_PIX_FMT_NONE);
    }
    ...
    public synchronized boolean recordImage(int width, int height, int depth, int channels, int stride, int pixelFormat,
        ...
        try (PointerScope scope = new PointerScope()) {
            ...
            if ((ret = avcodec_send_frame(video_c, image == null || image.length == 0 ? null : picture)) < 0
                ...
                throw new Exception(
                    "avcodec_send_frame() error " + ret + ": Error sending a video frame for encoding.");
            }
